...
const allowedOrigins = Array.from(new Set([...env.CORS_ORIGIN.split(",").map((s) => s.trim()).filter(Boolean), "http://localhost:5173"]));
...
app.post("/v1/payments/verify", async (req, res) => {
    ...
    const providerUsed = !isFailure && razorpay && data.razorpay_payment_id ? "razorpay" : "mock";
